...
void testRepo()
{
	...
	assert(*r.readEntity(1) == p1);
	assert(*r.readEntity(2) == p2);
	Player p3 = p2;
	r.deleteEntity(2);
	assert(r.getSize() == 1);
	r.updateEntity(1, p3);
...
void testService()
{
	...
	assert(*serv.readPlayer(1) == p1);
	assert(*serv.readPlayer(2) == p2);
	Player p3 = p2;
	serv.deletePlayer(2);
	assert(serv.getSize() == 1);
